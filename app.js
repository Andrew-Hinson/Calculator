...
body.addEventListener('click', activeFontSizeChanger);
body.addEventListener('keydown', activeFontSizeChanger);
//rewrite to - on enter or opperand, parse keyVal to integer
document.addEventListener('keydown', (e) => {
    if(e.key >= 0 && e.key <= 9){
        if(numToBeEvaled.length >= 19){
            numbers.attributes.disabled = true;
        }
        keyVal = e.key              //needs to be inside first conditional, only want values
